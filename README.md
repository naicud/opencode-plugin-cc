...
- **delegate** — resolves model+variant client-side (the server accepts any variant string and silently falls back to base — see `docs/opencode-api-findings.md` P2), creates a titled session, fires `prompt_async` with the work contract from `config/models.json`, records a job visible to `status`. Extras: `persona` picks the injected server-side agent (`builder` = full write access; `reviewer` = read-only, may only write `.oc-report.md` — its edit attempts surface as pending permissions), `retryOf` links a re-run to a failed/cancelled job, `resumeSessionID` continues an existing persisted session (crash recovery / multi-step), `account` routes quota across pooled accounts, `title` overrides the session name, `autoRetry: true` re-delegates ONCE at the escalation-suggested model+variant if the run dies with a retryable error (returns `status:"retried"` + new sessionID), `skills` injects up to 3 plugin skill digests into the worker prompt (spawn-time defaults are already baked into `oc-delegate`). Enforced guards: `config.concurrency.maxDelegates` cap and `config.budget` spend limits (`DELEGATE_LIMIT_EXCEEDED`, `BUDGET_JOB_MAX`, `BUDGET_DAILY_MAX`).
- **fanOut** — batch parallelism: delegate up to 12 tasks with ONE call. Same resolved model+variant for every task; round-robin account rotation per task (quota amplification); one job record each under a shared `fanOutId`; mid-loop failures keep started tasks running and are reported (`started`/`failed`). Tasks may be plain strings or `{task, cwd}` objects — per-task workspaces enable cross-repo parallel delegation. Response includes ready-made `nextStep` waitAll guidance and echoes any selected `skills`. **`mode:"race"`**: same task (or variants) to multiple sessions — the first clean completion wins, every other session is aborted and marked cancelled (`race-loser`); returns the winner's response + cost for quality-at-speed or cross-model comparison.
- **wait** — polls every 5s; returns on idle, on pending permission (`needsInput`), or timeout. **Two visibility channels, choose by cost:**
  - **Progress frames (default, ZERO tokens)**: with `_meta.progressToken` (Claude Code sends it automatically) the call blocks up to 600s while streaming a live one-line feed — reasoning · current tool call · assistant tail — every ~8s straight into Claude Code's UI. UI-only: nothing enters the model context, so watching the subagent costs no quota. **Long tasks chain**: at 600s the call returns `timeout` with a snapshot and `nextStep` — the session keeps running server-side, just call `wait` again (and again) until `idle`; nothing is lost.
  - **Auto-slice fallback (cheap context)**: without a progressToken the call returns every 60s with a slim snapshot (assistant tail, reasoning tail, last 3 tool calls, todos) plus `nextStep`; keep calling until `idle`. Override cadence with `OPENCODE_WAIT_SLICE_SEC`.
...
command succeeds, `.oc-report.md` mandatory). Plugin skills travel across the delegation boundary
the same way Claude Code injects an agent's frontmatter `skills:` into its subagents: selected
skill digests are baked into the `oc-delegate` prompt at spawn time (`opencode-prompting` by
default — see `mcp/lib/skills.mjs`), and `delegate`/`fanOut` accept an opt-in `skills` array
(max 3, validated before any server spawn) whose digests are appended to the task prompt under a
`[PLUGIN SKILLS]` marker — reaching also sessions that fall back to stock `build`.
`delegate`/`fanOut` run under the injected agent automatically and
