...
- **wait** — polls every 5s; returns on idle, on pending permission (`needsInput`), or timeout (with `progress`: latest assistant text tail + todo counts). Responses carry `jobId`, `account` and a todo summary so no extra calls are needed. With `_meta.progressToken` it streams MCP progress frames carrying the live assistant output (SSE `message.part.updated` tracker; interval via `OPENCODE_PROGRESS_INTERVAL_MS`, default 15s). **Auto-slice live feed**: without an explicit `timeoutSec` the call returns every 60s with a fresh snapshot — assistant tail, reasoning tail, last tool calls, todos — plus `nextStep`; keep calling `wait` until `idle` and the delegated agent streams like a classic subagent, no manual `logs` needed.
